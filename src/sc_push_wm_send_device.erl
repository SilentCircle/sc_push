...
malformed_request(ReqData, Ctx) ->
    case sc_push_wm_common:ensure_path_items([device_id], ReqData) of
        {ok, Props} ->
            Ctx2 = Ctx#ctx{device_id = proplists:get_value(device_id, Props)},
            {false, ReqData, Ctx2};
        {error, Msg} ->
            RD2 = sc_push_wm_helper:malformed_err(Msg, ReqData),
            {true, RD2, Ctx}
...
process_post(ReqData, #ctx{method = 'POST',
    ...
    case ParseRes of
        {ok, Props0} ->
            Receivers = {receivers, [{device_id, [sc_util:to_bin(Device)]}]},
